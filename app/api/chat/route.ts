  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: 'API Key 未設定', message: '抱歉，AI 服務目前無法使用。請聯繫我們獲取協助：https://lin.ee/ZTgJbYG' },
        { status: 500 }
      );
    }
...
語氣要專業、具備前瞻性，並能精準分析用戶對自動化工具的需求。`,
    ...
    return NextResponse.json({
      text: response.text || "抱歉，我現在無法回答，請稍後再試。",
      hasLinks: false,
    });
  } catch (error: any) {
    console.error("Chat API Error:", error);
    return NextResponse.json(
      { error: 'AI 連線異常', message: 'AI 連線異常，請檢查網路或稍後再試。' },
      { status: 500 }
    );
  }
}
